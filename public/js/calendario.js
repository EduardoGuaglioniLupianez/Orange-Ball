...
function _renderizar_calendario(datas_com_jogo) {
    ...
    for (let i = 1; i <= qtd_dias_valor; i++) {
        ...
        if (tem_jogo) {
            id_dias.innerHTML += `
                <button class="dia_button" onclick="abrirModal('${chave}')" title="Ver jogo!">
                    ${i}
                    <span style="font-size:16px; display:block; line-height:1;">🏀</span>
                    <video class="video_fundo" autoplay loop muted playsinline>
                        <source src="assets/imgs/Kobebackgroudhome.gif" type="video/mp4">
                    </video>
                </button>`
        } else {
            id_dias.innerHTML += `
                <button class="dia_button">
                    ${i}
                    <video class="video_fundo" autoplay loop muted playsinline>
                        <source src="assets/imgs/Kobebackgroudhome.gif" type="video/mp4">
                    </video>
...
function abrirModal(chave_data) {
    ...
    fetch('/stats/porData/' + fk_usuario + '/' + chave_data)
        ...
        .then(function(jogo) {
            ...
            let imgEl   = document.getElementById("modal_img")
            let semFoto = document.getElementById("modal_sem_foto")
            if (jogo.tem_imagem) {
                imgEl.src             = "/stats/imagem/" + jogo.id
